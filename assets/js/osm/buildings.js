...
import {getBoundingBoxString, fetchFromOverpass, tileposFromLatlon, latlonFromJSON, getRelativePositionFromTilepos, getPositionFromTilepos} from './globals.js'
...
var specialDefaults = {
  construction: {"building:colour": "#fefefe"},
...
function loadBuildings() {
  ...
  return fetchFromOverpass(opQuery)
    .then((itemJSON) => {
      var count = 0;
      for (var feature of itemJSON.features) {
...
function addBuilding(jsonFeature) {
  return new Promise((resolve, reject) => {
    ...
    let buildingProperties = {primitive: "building", outerPoints: outerPoints};
    if (innerWays.length) {
...
AFRAME.registerGeometry('building', {
  ...
  init: function (data) {
    ...
    if (data.innerPaths.length) {
      for (var ipoints of data.innerPaths) {
